...
The spec was generated with `rust2rpm 28` and then reviewed and adapted by Marcin FM. Its source and noarch development RPMs pass a clean Fedora 44 mock build using `rust-dirs-sys0.4` from the same package chain. It is enabled only for `fedora-44-x86_64`; Fedora 43 already provides this crate branch and must not receive a duplicate build.
