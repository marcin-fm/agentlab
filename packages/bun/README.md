...
Release `0.0.38` adds the byte-identical upstream MIT text for selected `peechy 0.4.34` as Source32. The signed npm archive declares MIT and names the same repository but omits a license file; its registry `gitHead` is unavailable and upstream issues are disabled, so exact release-source correspondence remains false. The checked source inventory now has supplied text for 223 of 236 npm archives, and all 38 selected code-generation packages have required text evidence. Six WebKit fork-file terms, final aggregate SPDX and RPM payload accounting, immutable relink delivery, aarch64 RPM acceptance, FPC bootstrap approval, and COPR remain unresolved.
