...
Bun's release source pins `nightly-2025-12-10` and `rust-src` only because its Linux release branch rebuilds the standard library with `panic=immediate-abort` for roughly 180 KB of size reduction. `bun-lolhtml-fedora-stable-rust.patch` excludes native Linux from that optional branch and retains upstream's existing stable `panic=abort`, no-unwind-table, size-optimized path. Fedora Rust/Cargo `1.96.1` rebuilt `lol_html_c_api` from a deterministic 43-crate vendor tree with `--locked --offline` inside a network namespace. The 46,769,250-byte direct-proof `liblolhtml.a` exports 97 checked C API symbols. `lolhtml-offline-build-proof.json` has SHA-256 `05785f79c560ce5953500acc7c9dea283a53b18240e59b62c84f57ab02eb5cce`. Release `0.0.18` now extracts the same source as `vendor/lolhtml`, writes Bun's checked `.ref` identity, stages `Source24` under `c-api`, and uses `%cargo_prep -v`, `%cargo_build`, and `%cargo_vendor_manifest`. A network-isolated rpmbuild proof produced a 66,214,202-byte Fedora-profile `liblolhtml.a`, the same 97-symbol API, 43 vendor source directories, and a path-normalized 41-package linked manifest. `lolhtml-rpm-cargo-proof.json` records that stage; nightly Rust and `rust-src` remain unnecessary for native Linux.

The 236 checked npm archives now reconstruct Bun's exact cache version 1 both in the retained proof and during RPM `%prep`. Bun `1.3.14` does not parse lockfile libc selectors, so `bun-lightningcss-fedora-glibc-lock.patch` disables only the incompatible musl Lightning CSS prebuilt package with Bun's supported `os: none` value while preserving the GNU package. The checked bootstrap seed then completed all three serialized `--frozen-lockfile --ignore-scripts` installs under `unshare --net`: 230 root packages, Preact for `packages/bun-error`, and 121 `src/node-fallbacks` packages. Release `0.0.19` does not import that seed; `bun-stage-release-local-sources` instead materializes the 236-entry cache directly from Source23 and reproduces the exact 4,613-entry, 121,972,064-byte tree at SHA-256 `50e66a5b8361735b2598a6be5d7d78f973db05104cbdf9b9addb01e9a113d214`. Two independent network-isolated `%prep` runs produced byte-identical staging receipts and cache manifests. `release-local-source-staging-proof.json` records this boundary; actual `node_modules` installation remains a source-built-driver step.

`bun-1.3.14-source-license-inventory.json` records the provisional source-license boundary without claiming the final linked payload. Fedora `cargo2rpm` reports eight expression classes for the lol-html graph, binds the verified 41-package manifest, and exposes one unresolved local `lol_html_c_api` declaration. The 236 npm sources declare 11 expression values; recursive discovery finds supplied license texts for 221 entries, 15 have no such text, and the two missing modern `license` fields both retain supplied MIT evidence. Picohttpparser explicitly selects the complete embedded MIT terms instead of its Perl alternative. The inventory also hashes the license candidates for all 19 native sources and 35 retained WebKit files. The standalone generator is carried as Source28 and reruns in network-isolated `%prep` against the actual staged tree. Final Fedora SPDX review, linked-component selection, required `%license` texts, aggregate `License:`, bundled Provides, and RPM payload validation remain false.
