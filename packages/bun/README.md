...
`bun-1.3.14-final-linked-license-closure.json` binds the final self-rebuild, relink audit, relink kit, current source-license inventory, and reviewed lol-html package metadata. It assigns all 1,162 direct objects to Bun or the 18 checked native source identities, binds the three exact WebKit archives, records the ten system link flags, and rejects unknown inputs. Fifteen native source-subset selections are verified; `libarchive`, `libjpeg-turbo`, and `libwebp` remain unresolved mixed-license trees with their additional notice and patent evidence retained. WebKit linked-file semantics, npm/codegen final-payload provenance, required texts, aggregate SPDX, and RPM payload verification remain explicitly unresolved; the shared lol-html provider is excluded from Bun's bundled aggregate.
