...
Release `0.0.42` binds the checked selected-license evidence for all 18 linked native components and all 38 selected npm/code-generation packages into the final-link closure, preserves the complete Fedora 44 aarch64 source-build and wrapper-free relink evidence, and promotes the nine generated headers' exact source and byte provenance from two independent network-isolated source-built generator runs. The signed peechy npm archive declares MIT and names the same repository but omits a license file; its registry `gitHead` is unavailable and upstream issues are disabled, so exact release-source correspondence remains false. Six WebKit fork-file terms, distinct Ninja producer edges and the broader npm/code-generation final-payload closure, aggregate SPDX and RPM payload accounting, immutable relink delivery, aarch64 RPM integration and acceptance, FPC bootstrap approval, and COPR remain unresolved.
...
Source22 is a deterministic 239-archive union of the checked x64 and arm64 npm closures: 41,222,363 bytes at SHA-256 `c8f251509523fe27a648e496df9d5a7ee564c07a8afc5d67c712489df2fee7b4`. The configured-SCM-equivalent `0.0.42` source-delivery proof regenerates both 255-input closures byte-identically, records their 258-input union, and verifies the 329,447,791-byte, 43-member source RPM at SHA-256 `f4110ca45ee2fd62cc8656920ea9d316b0d8cb758cc14336a48f0014dfd31616`. Two network-isolated `%prep` runs reproduce the existing 236-entry npm cache and source staging, semantically reproduce the source-license inventory, validate the immutable generator-reexecution proof, and checksum-verify the selected-license receipts, preserving the 4,613-entry, 121,972,064-byte cache tree at SHA-256 `50e66a5b8361735b2598a6be5d7d78f973db05104cbdf9b9addb01e9a113d214`. `%prep` remains explicitly x64-only; Source31 is source-delivery evidence only and makes no arm64 `%prep`, build, payload, or COPR claim.
...
`bun-1.3.14-final-linked-license-closure.json` binds the final self-rebuild, relink audit, relink kit, current source-license inventory, selected npm/code-generation closure, and reviewed lol-html package metadata. It assigns all 1,162 direct objects to Bun or the 18 checked native source identities, records the ten system link flags, and rejects unknown inputs. The exact selected native expression set and all 23 checked native license files are verified; the exact 38-package npm/code-generation expression set and all 38 required texts are also verified. The two source-built reexecution runs establish the nine generated headers' exact source and byte provenance without claiming distinct Ninja outputs or the broader final npm/code-generation payload closure. The Bun-authored JPEG stub remains under Bun's MIT component, 62 upstream JPEG objects select `IJG AND BSD-3-Clause`, and 123 WebP objects select `BSD-3-Clause WITH Google-Patent-WebM`. The receipt cross-checks the three exact WebKit archives against `build.ninja`, `compile_commands.json`, unified-source includes, and Ninja's retained dependency database: 531 archive objects resolve to 5,098 unique WebKit-local dependencies, including 4,196 source and 902 generated paths. Three headerless subordinate/generated records are resolved under exact MIT evidence; six Bun-authored fork additions remain without explicit terms and retain their introducing commits as an upstream clarification hold. WebKit semantic selection, distinct generated-output edges, the broader npm/codegen final-payload closure, package-wide required texts, aggregate SPDX, and RPM payload verification remain explicitly unresolved; the shared lol-html provider is excluded from Bun's bundled aggregate.

`bun-1.3.14-npm-code-generation-closure.json` maps the current code-generation install boundary to 38 exact package manifests and maps Ninja's retained dependencies from all 1,162 direct link objects to 30 generated outputs consumed by 37 objects. Thirty-seven selected packages carry package-local license texts. `constants-browserify 1.0.0` supplies its complete MIT notice in the exact release README; `peechy 0.4.34` uses the separately checked Source32 canonical MIT text while its unavailable registry `gitHead` keeps exact release-source correspondence false. Nine generated headers are consumed by the final link without distinct Ninja producer edges; the v2 receipt binds them to the exact upstream orchestrator, grouped bindgen or bindgenv2 edge, generator, helper, semantic definition files, and retained output bytes. Two independent network-isolated runs remove and recreate all 26 expected outputs with the exact source-built Bun driver and generators, preserving the complete mode-aware 35,656-entry source tree and reproducing the complete mode-aware 118-entry codegen tree byte-identically while rejecting unsupported filesystem entries. Generated-header source and byte provenance is verified independently; distinct Ninja output edges and the broader final npm/codegen payload closure remain false rather than inferred.
