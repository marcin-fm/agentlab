...
The checked seed drove the first complete `release-local` build under `unshare --net`, with one `nice` four-job Ninja graph and matching nested CMake/Cargo caps. Two narrow Fedora corrections were required: `bun-zig-build-cwd.patch` runs out-of-tree Zig subprocesses from the source root, and `bun-fedora-shared-cxx-runtime.patch` links Fedora's shared `libstdc++` and `libgcc_s` instead of unavailable static runtime archives. The build produced a 336,047,704-byte `bun-profile`, a 67,721,320-byte debug-section-free `bun`, and a retained linker map. Revision, version, and JavaScript smokes passed; all 19 native sources, Node headers, stable lol-html Cargo, local WebKit, and three prevalidated npm roots plus their workspace-linked supplemental tree were consumed without network access. Resume mode revalidates those prepared inputs, and the generated graph restricts seed use to an exact checked 16-rule bootstrap scope with no active cross-target Rust or Zig-fetch edge. Scans found no seed hash/path in source, cache, or build outputs, while `ldd` confirmed Fedora `libstdc++.so.6` and `libgcc_s.so.1` with no seed dependency. `first-source-build-proof.json` has SHA-256 `fda7910119f4c11a2b7b11f19dc1f04a2315d580c192e4a007d6e654f27804eb`. A separate read-only audit resolves the final Ninja link edge to 1,162 direct objects totaling 1,463,081,632 bytes, four static archives, two linker scripts, and 2,294 generated WebKit headers. It also proves that Ninja declared but did not retain the response file and that the retained link rule still invokes the bootstrap seed wrapper. `relink-materials-proof.json` has SHA-256 `cc8acb65b728cac6e867f01b6e5c461e91879b28c420789a140f9663cec7b068`; it deliberately keeps complete LGPL relink materials and final legal/RPM acceptance false.
