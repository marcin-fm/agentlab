...
`.github/workflows/release-source.yml` recreates the complete archive from the pinned Git commit, regenerates the minimized source, checks it against package metadata and checked receipts, uploads a non-overwriting draft release, re-downloads every asset, and attaches GitHub provenance. It deliberately leaves the release as a draft because GitHub exposes immutable-release enablement only through repository settings; publish the verified draft only after that setting is enabled.
