...
`bun-1.3.14-npm-code-generation-closure.json` maps the current code-generation install boundary to 38 exact package manifests and maps Ninja's retained dependencies from all 1,162 direct link objects to 30 generated outputs consumed by 37 objects. Thirty-six selected packages carry package-local license texts; `constants-browserify 1.0.0` and `peechy 0.4.34` declare MIT but omit the required text from their exact npm archives. Later repository texts are recorded with exact commits and hashes, but exact release-source correspondence remains false. Nine generated headers are consumed by the final link without distinct Ninja producer edges, so producer-edge completeness and the final npm/codegen closure remain false rather than inferred.
