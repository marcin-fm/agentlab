...
`bun-1.3.14-final-linked-license-closure.json` binds the final self-rebuild, relink audit, relink kit, current source-license inventory, and reviewed lol-html package metadata. It assigns all 1,162 direct objects to Bun or the 18 checked native source identities, binds the three exact WebKit archives, records the ten system link flags, and rejects unknown inputs. All 18 native source-subset selections are verified, including libarchive's exact `BSD-2-Clause AND BSD-3-Clause AND CC0-1.0` linked expression. The Bun-authored JPEG stub remains under Bun's MIT component, 62 upstream JPEG objects select `IJG AND BSD-3-Clause`, and 123 WebP objects select `BSD-3-Clause WITH Google-Patent-WebM`. WebKit linked-file semantics, npm/codegen final-payload provenance, required texts, aggregate SPDX, and RPM payload verification remain explicitly unresolved; the shared lol-html provider is excluded from Bun's bundled aggregate.
