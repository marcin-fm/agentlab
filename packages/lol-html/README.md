...
The retained Fedora 44 x86_64 Mock and configured-SCM COPR evidence is immutable historical proof for `3.0.0`: all eight upstream C suites, the focused C smoke, SONAME, pkg-config, 97-symbol, `ldd -r`, digest, payload, and six-cell matrix checks passed. It is not relabeled as `3.0.1` build evidence. The `3.0.1` increment instead statically verifies the unchanged C API `1.4.0`, header, SONAME, symbol expectation, dependency closure, and linked-license contract; fresh target build proof remains the next integration step. No RPM was installed.
