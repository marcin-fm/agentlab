...
| agent-browser | 0.33.1 | enabled | Six-cell Fedora headless Chromium matrix, linked-license payload, wrapper routing, and Fedora/RPM Fusion duplicate audit pass at 0.15; 0.16 separates static checks from dynamic COPR evidence |
...
| python-docling-slim | 2.117.0 | enabled | API-only service-client update; Core 2.88 remains compatible, chunking and multi-target APIs are checked, and the six-cell matrix remains required |
...
| python-headroom-ai | 0.33.0 | enabled | Release 0.6 selects exact tree-sitter/tokenizers providers, a checked Fedora-family linked-license contract, a reproducible fixture-only source from the exact upstream commit, and deterministic non-ML test timing; current six-target configured-SCM matrix required |
