...
| Capability | Exact source and dependency edge | Fedora 43/44 and RPM Fusion status | Missing reusable packages | System/native implications | Runtime network/model behavior | License/source closure | Smallest offline test | Outcome |
|---|---|---|---|---|---|---|---|---|
| Full `@playwright/mcp 0.0.78` | npm wrapper plus `playwright`/`playwright-core 1.62.0-alpha-1783623505000`; generated esbuild/Vite runtime | No exact Fedora/RPM Fusion MCP or Playwright provider; metadata requests Chromium 151, but the exact wrapper passed an stdio 24-tool navigation/evaluation smoke with Fedora 44 Chromium 150. Fedora source-builds esbuild 0.24.2/0.27.2, not the pinned 0.28.1. | Full 679-tarball root closure, separate stable-test-runner closure, exact source-built esbuild 0.28.1 provider, aggregate SPDX accounting, and immutable hosting | Browser process, sandbox, broad generated Node runtime, and copied `xdg-open` | Browser automation; Playwright-managed browser downloads must remain disabled | All seven wrapper files match the release tag and five bundle-license sidecars map to exact lock versions. Exact esbuild is source-buildable through the upstream/Fedora pattern, but Playwright exposes no MCP-only build target and the generated payload is not reproduced offline. | Complete offline monorepo source build, then repeat the packaged stdio navigation/evaluation smoke using Fedora `chromium-headless` in both target chroots | **blocked** on monorepo source/build/license closure, not Chromium or an inherent prebuilt-esbuild requirement; do not hand-write a reduced runtime |
