...
| Capability | Exact source and dependency edge | Fedora 43/44 and RPM Fusion status | Missing reusable packages | System/native implications | Runtime network/model behavior | License/source closure | Smallest offline test | Outcome |
|---|---|---|---|---|---|---|---|---|
| Full `@playwright/mcp 0.0.78` | npm wrapper plus `playwright`/`playwright-core 1.62.0-alpha-1783623505000`; generated esbuild/Vite runtime | No exact Fedora/RPM Fusion MCP or Playwright provider; Fedora Chromium is 150.0.7871.114, while metadata requires Chromium 151.0.7922.10 revision 1232 | Reproducible generated-runtime closure and exact browser compatibility proof | Browser process, sandbox, native Node/runtime dependencies | Browser automation; Playwright-managed browser downloads must remain disabled | Wrapper correspondence is recorded, but generated assets, licenses, and complete source reconstruction are unresolved | Complete offline source build, then stdio initialize plus local `data:` navigation using Fedora `chromium-headless` with downloads disabled | **blocked**; do not hand-write a reduced runtime |

## Completed Wave Gates

The Docling remote-conversion package was the sole selected wave. All of the
following gates passed before it was enabled:
