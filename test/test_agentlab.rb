...
class AgentlabTest < Minitest::Test
  ...
  def test_xberg_is_blocked_and_kreuzberg_is_not_wired_for_source_generation
    ...
    assert_includes(spec, "# Fedora's workspace-wide %%cargo_vendor_manifest")
    refute_includes(spec, "# Fedora's workspace-wide %cargo_vendor_manifest follows")
    ...
    assert_equal([604, 1_133, 529, 610, 629, 0, -2, 529], contract.values_at("selected_registry_identities", "vendor_registry_identities", "resolver_only_registry_identities", "normal_packages", "normal_build_dev_packages", "git_dependencies", "ort_download_delta", "resolver_only_additions"))
    ...
    assert_equal([10_786_324, "fedora-44-x86_64", "xberg-1.0.1-0.15", "9da0f4b96a2c125cdc2ac83031b3776d26108c92e69e1e43e9361cc293168971", 195_601_069, 13], source_proof.values_at("copr_build_id", "target", "source_nvr", "srpm_sha256", "srpm_size_bytes", "srpm_members"))
    assert(source_proof.values_at("configured_scm_source_generated", "source_hashes_and_patches_verified", "vendor_tree_verified", "cargo_offline_configuration_verified", "intentional_prep_blocker_reached", "temporary_copr_definition_deleted").all?)
    ...
    assert_equal("root-lock-registry-complete", closure.dig("resolver_model", "kind"))
    assert_equal([604, 1_133, 529], closure.fetch("counts").values_at("selected_registry_identities", "vendor_registry_identities", "resolver_only_registry_identities"))
    ...
    assert_equal("b0d603d84a23e6f58590660220edf82bfc5152269419abb7084fd831bb4762f2", vendor_receipt.dig("vendor_tree", "sha256"))
    assert_equal([57_079, 10_432, 0, 1_211_004_693], vendor_receipt.fetch("vendor_tree").values_at("files", "directories", "symlinks", "bytes"))
    license = JSON.parse(File.read(File.join(package.directory, contract.dig("source_evidence", "license_text_presence", "file"))))
    assert_equal({ "present" => 1_043, "absent" => 90 }, license.fetch("counts"))
    assert_includes(spec, "%cargo_prep -v cargo-vendor")
    assert_includes(spec, "audit-xberg-cargo-closure --verify")
    ...
    assert_equal("60557740a675908ea8d27145841dafd4f6a06917", receipt.dig("source", "commit"))
    assert_equal("19705db11615dd4d1722dec9224c3b752cd369af", receipt.dig("source", "tree"))
    ...
    assert_equal({ "prior_reported_count" => 45, "revalidated_for_xberg" => false }, receipt.dig("prior_audit_notes", "reusable_exact_identities"))
    assert_equal(17, receipt.dig("prior_audit_notes", "conservative_retirement_candidates", "prior_reported_count"))
    assert_equal(63, receipt.dig("prior_audit_notes", "root_lock_presence_cross_check", "historical_package_rows"))
    assert_equal(42, receipt.dig("prior_audit_notes", "root_lock_presence_cross_check", "exact_crate_name_version_pairs_present_in_xberg_root_lock"))
    assert_equal(false, receipt.dig("prior_audit_notes", "root_lock_presence_cross_check", "retirements_authorized"))
    assert_equal(5, receipt.dig("prior_audit_notes", "version_review_notes").length)
    assert_equal(10, receipt.dig("prior_audit_notes", "new_identity_notes").length)
    assert_equal("system/dynamic ORT", receipt.dig("native_boundaries", "onnx_runtime"))
    assert_equal("Complete offline Cargo metadata stopped on missing auto_enums 0.8.10.", receipt.dig("offline_cargo_metadata", "limitation"))
    assert(receipt.fetch("validation").values.all? { |value| value == false })
    assert_equal(package.data.dig("system_ort_audit_receipt", "sha256"), Digest::SHA256.file(system_ort_receipt_path).hexdigest)
    assert_equal("agentlab-xberg-system-ort-audit/v1", system_ort_receipt.fetch("schema"))
    assert_equal(610, system_ort_receipt.dig("graph", "normal_edges", "baseline_packages"))
    assert_equal(610, system_ort_receipt.dig("graph", "normal_edges", "patched_packages"))
    assert_equal(-2, system_ort_receipt.dig("graph", "normal_build_dev_union", "delta"))
    assert_equal(false, system_ort_receipt.dig("ort_feature_resolution", "ort_sys_download_binaries_patched"))
    assert_equal(false, system_ort_receipt.dig("ort_feature_resolution", "hmac_sha256_patched"))
    assert_equal(true, system_ort_receipt.dig("ort_feature_resolution", "lzma_rust2_0_16_5_patched"))
    assert(system_ort_receipt.fetch("validation").values_at("vendor_source_delivery", "provider_matrix", "compilation", "ort_runtime_smoke", "models_licenses", "aggregate_license", "enablement", "copr", "retirements").all? { |value| value == false })
