...
class AgentlabTest < Minitest::Test
  ...
  def test_headroom_mcp_and_jwt_current_static_contracts
    ...
    assert_equal("==2.0.0", mcp_2_audit.dig("dependencies", "candidate_2_0_0", "mcp-types"))
  ...
  def test_xberg_is_blocked_and_kreuzberg_is_not_wired_for_source_generation
    ...
    assert_equal("5351df360176c53ba3ac2cf618939b4c1408a5dd58945bbf9f66df0cc4709b76", package.data.dig("source_audit_receipt", "sha256"))
    ...
    assert_equal("e35f9270a14928db8271c7712af4fbc26395670f5f68543858ea20cb8532f2c4", package.data.dig("cargo_source_contract", "source_evidence", "lightweight_model_source", "sha256"))
    assert_equal("0f99bf2a4adc592838b4a446c89f0a35f75b68a21ef5d7a70b83be510586919b", package.data.dig("cargo_source_contract", "source_evidence", "model_source_auditor", "sha256"))
    assert_equal("ca8b4e16be0c1bdee3f9d61b39f281503c9ca430da5075616d190eed38c053ef", package.data.dig("cargo_source_contract", "source_evidence", "model_released_witness", "sha256"))
    assert_equal("c89ffc17e03f0da7e14647d24101eb0ff3894bbdf8185e41de1ee1df3ce674e1", package.data.dig("cargo_source_contract", "source_evidence", "model_mirror_tree", "sha256"))
    assert_equal("f8fa3acf4134b56b6d1b1f6facc60ac8b94266fc2fa5e2d3e79152101938e75e", package.data.dig("cargo_source_contract", "source_evidence", "model_upstream_tree", "sha256"))
    assert_equal("ef2bf6196b9229e9313c6aac805a903fdd96b140a6540629d013ac790ef1b298", package.data.dig("cargo_source_contract", "source_evidence", "model_upstream_readme", "sha256"))
    ...
    assert_includes(spec, "Release:        0.9%{?dist}")
    ...
    assert_includes(spec, "Source21:       %{name}-%{version}-lightweight-released-source.json")
    assert_includes(spec, "Source22:       %{name}-embedding-models-4b127809-potion-tree.json")
    assert_includes(spec, "Source23:       minishlab-potion-base-8M-bf8b0566-tree.json")
    assert_includes(spec, "Source24:       minishlab-potion-base-8M-bf8b0566-README.md")
    ...
    assert_equal("agentlab-xberg-model-source/v2", model_receipt.fetch("schema"))
    assert_equal("bf8b056651a2c21b8d2565580b8569da283cab23", model_receipt.dig("upstream", "revision"))
    assert_equal(%w[config.json model.safetensors special_tokens_map.json], model_receipt.dig("mirror", "provided_files").map { |file| file.fetch("upstream_path") })
    assert_equal(%w[tokenizer.json tokenizer_config.json vocab.txt], model_receipt.dig("upstream", "upstream_only_files").map { |file| file.fetch("path") })
    assert(model_receipt.dig("upstream", "upstream_only_files").all? { |file| file.fetch("released_download_path_established") == false })
    assert_equal(false, model_receipt.dig("claims", "released_runtime_file_set_available_at_mirror_revision"))
    assert_equal(false, model_receipt.dig("claims", "released_upstream_direct_fallback_established"))
    assert_equal(false, model_receipt.dig("claims", "lightweight_preset_source_identity_complete"))
    assert_equal(false, model_receipt.dig("claims", "conversion_required_for_mirror_model"))
    ...
    assert_equal(false, model_receipt.dig("claims", "redistribution_review_complete"))
    ...
    assert_equal("1.0.3-0.9", receipt.fetch("current_package_version"))
    ...
    assert_equal({
      ...
      "lightweight_model_source" => { "filename" => "xberg-1.0.3-lightweight-model-source.json", "sha256" => "e35f9270a14928db8271c7712af4fbc26395670f5f68543858ea20cb8532f2c4" }
  ...
  def test_validates_bun_dependency_staging_receipt
    ...
    Dir.mktmpdir do |directory|
      data = Marshal.load(Marshal.dump(package.data))
      copied_stages = data.dig("build_plan", "stages")
      dependency_stage = copied_stages.fetch("dependency_staging")
      dependency_receipt_name = dependency_stage.fetch("proof_receipt")
      dependency_receipt_path = File.join(directory, dependency_receipt_name)
      dependency_receipt = JSON.parse(File.read(File.join(package.directory, dependency_receipt_name)))
      dependency_receipt.fetch("prep").delete("final_linked_license_receipt_checksum_verified")
      File.write(dependency_receipt_path, JSON.pretty_generate(dependency_receipt) + "\n")
      dependency_stage["proof_receipt_sha256"] = Digest::SHA256.file(dependency_receipt_path).hexdigest

      source_delivery_name = copied_stages.fetch("source_delivery").fetch("proof_receipt")
      FileUtils.cp(File.join(package.directory, source_delivery_name), File.join(directory, source_delivery_name))
      copied_package = Agentlab::Package.new(directory: directory, manifest_path: "unused", data: data)

      errors = Agentlab.validate_bun_dependency_staging(
        copied_package,
        dependency_stage,
        copied_stages.fetch("source_delivery"),
        copied_stages.fetch("dependency_closure"),
        data.dig("build_plan", "source_inputs", "release_local_staging"),
        "1.3.14",
        spec
      )
      assert_includes(errors, "bun: dependency-staging final linked-license receipt checksum was not verified")
    end
  end
  ...
  def test_validates_bun_final_linked_license_closure
    ...
    Dir.mktmpdir do |directory|
      ...
      %w[
        ...
        bun-1.3.14-npm-code-generation-closure.json
      ...
      receipt.dig("selected_license_evidence", "bundled_native")["selected_expression_counts"] = { "MIT" => 18 }
      receipt.dig("selected_license_evidence", "npm_codegen_inputs")["packages_with_required_text"] = 37
      ...
      assert_includes(errors, "bun: final linked-license selected native SPDX evidence mismatch")
      assert_includes(errors, "bun: final linked-license selected npm text evidence mismatch")
