...
class BunWebKitSourceTest < Minitest::Test
  ...
  def test_packages_a_deterministic_reduced_source_tree_and_verifies_its_receipt
    Dir.mktmpdir("agentlab-webkit-source-test-", "/srv/tmp") do |temporary|
      ...
      pycache = File.join(extracted_root, "Source", "JavaScriptCore", "__pycache__")
      FileUtils.mkdir_p(pycache)
      File.binwrite(File.join(pycache, "generator.cpython-314.pyc"), "generated bytecode")
      ...
      File.write(File.join(pycache, "unexpected.txt"), "not generated bytecode")
      error = assert_raises(Agentlab::Error) do
        Agentlab::BunWebKitSource.verify_tree!(staging: extracted, receipt: verified)
      end
      assert_includes(error.message, "does not match its receipt")
      FileUtils.rm_f(File.join(pycache, "unexpected.txt"))
