...
Authenticated automation uses the explicit identity-scoped path in `COPR_CONFIG`, not an ambient `HOME`. Before any project or package mutation, the scripts require `copr-cli whoami` to match the configured owner `marcin` exactly.
